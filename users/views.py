...
def Profile(request):
    ...
    if request.method == 'POST':
        ...
        if u_form.is_valid() and p_form.is_valid():
            u_form.save()
            p_form.save()
            messages.success(request, 'acount register succesfully')
...
def kerkesa(request):
    if request.method == 'POST':
        ...
        send_mail(
            'tution wala',
            'thanku for contacting tution wala' + emri + ' , ' + email + ' , ' + numri_tel + ' , ' + str(prof) + '.',
            'ashishrohilla510@gmail.com',
            ['Ashish@321'],
            fail_silently=False,
        )
        messages.info(request, f'check your email for teacher signup')
